...
var moveChildUp = function moveChildUp(blockMap, key) {
  ...
  if (childIndex === 0) {
    ...
    if (parent.getChildKeys().count() > 0) {
      ...
      newBlockMap = newBlockMap.set(firstChildKey, firstChild.merge({ prevSibling: null }));
    }
    ...
    if (parent.getChildKeys().count() > 0) {
      ...
      newBlockMap = newBlockMap.set(lastChildKey, lastChild.merge({ nextSibling: null }));
    }
  }
  ...
  if (grandparentKey != null) {
    ...
    newBlockMap = newBlockMap.set(key, newBlockMap.get(key).merge({ parent: null }));
  }
  ...
  if (parent.getChildKeys().count() === 0) {
    ...
    if (prevSiblingKey == null && nextSiblingKey != null) {
      newBlockMap = newBlockMap.set(nextSiblingKey, newBlockMap.get(nextSiblingKey).merge({ prevSibling: null }));
    }
    if (nextSiblingKey == null && prevSiblingKey != null) {
      newBlockMap = newBlockMap.set(prevSiblingKey, newBlockMap.get(prevSiblingKey).merge({ nextSibling: null }));
    }
    if (grandparentKey != null) {
...
var mergeBlocks = function mergeBlocks(blockMap, key) {
  ...
  if (nextNextBlockKey != null) {
    newBlockMap = newBlockMap.set(nextNextBlockKey, blockMap.get(nextNextBlockKey).merge({ prevSibling: key }));
  }
  verifyTree(newBlockMap);
  return newBlockMap;
};
