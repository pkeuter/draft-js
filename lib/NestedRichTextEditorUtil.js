  ...
  onTab: function onTab(event, editorState, maxDepth) {
    ...
    if (!event.shiftKey) {
      ...
      if (prevSiblingNonLeaf) {
        blockMap = DraftTreeOperations.updateAsSiblingsChild(blockMap, key, 'previous');
        ...
        blockMap = DraftTreeOperations.updateAsSiblingsChild(blockMap, key, 'next');
    ...
    if (selection.isCollapsed() && offset === 0) {
      ...
      if (type !== 'unstyled') {
        if ((type === 'unordered-list-item' || type === 'ordered-list-item') && depth > 0) {
          ...
          newBlockMap = newBlockMap.set(key, newBlockMap.get(key).merge({ depth: depth - 1 }));
...
var onUntab = function onUntab(blockMap, block) {
  ...
  if (blockIndex === 0 || blockIndex === existingChildren.count() - 1) {
    ...
    blockMap = blockMap.map(function (block) {
      return nextChildren.includes(block.getKey()) ? block.merge({ parent: newBlock.getKey() }) : block;
    });
    // update the next/previous pointers for the children at the split
    blockMap = blockMap.set(key, block.merge({ nextSibling: null })).set(nextSiblingKey, blockMap.get(nextSiblingKey).merge({ prevSibling: null }));
